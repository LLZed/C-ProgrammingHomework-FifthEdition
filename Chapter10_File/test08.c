...
int main()
{
    ...
    readfile("test8stud_sort.txt", "r", Stu, N);
    print(Stu, N);
    int i,t;
    for(i=0;s.ave < Stu[i].ave && i<N;i++);//执行空语句来计数i，找到插入位置
    inputfile("test8stud_sort.txt", "w", Stu, i);//把比插入的学生的平均成绩大的先写入文件
    inputfile("test8stud_sort.txt", "a", p, 1);//用追加形式打开文件，不然会刷新文件内容，然后把要插入的学生写入文件
    inputfile("test8stud_sort.txt", "a", &Stu[i], N-i);//再把剩下的数据也写入文件
