...
int main()
{
    ...
    struct Student *del(struct Student *p,long num);    //函数声明
    struct Student *insert(struct Student *p,struct Student *stud);    //函数声明
    void print(struct Student *p);                      //函数声明

	struct Student *pt,*pt1,*pt2;					     //定义一个结构体指针变量；
    struct Student *stud;					             //定义一个结构体指针变量；
    char c;
    long number;
	pt2=pt1=pt=creat();								             //调用函数，函数返回所创建的链表的第1个结点的地址，用三个指向
    print(pt);
...
struct Student *del(struct Student *p,long num)
{
    ...
    {
        p2=p1;p1=p1->next;					//p1后移一个结点，指向下一个结点
    }
